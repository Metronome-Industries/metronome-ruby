...
class Metronome::Test::BaseModelTest < Minitest::Test
  ...
  def test_basic_coerce
    assert_equal(
      [1, 2, 3],
      Metronome::Converter.coerce(A1, [1.0, 2.0, 3.0])
    )
    assert_equal(
      [:a, :b, :c],
      Metronome::Converter.coerce(A2, %w[a b c])
    )
  end
  ...
  def test_basic_dump
    assert_equal(
      [1, 2, 3],
      Metronome::Converter.dump(A1, [1.0, 2.0, 3.0])
    )
    assert_equal(
      [:a, :b, :c],
      Metronome::Converter.dump(A2, %w[a b c])
    )
  end
  ...
  def test_basic_model_coerce
    ...
    cases.each do |input, output|
      assert_equal(output, Metronome::Converter.coerce(M2, input))
    end
  end

  def test_basic_model_dump
    assert_nil(Metronome::Converter.dump(M2, nil))
    assert_equal({}, Metronome::Converter.dump(M2, {}))
    assert_equal({w: 1, x: "x"}, Metronome::Converter.dump(M2, {w: 1, x: "x", y: 1, z: "z"}))
    assert_equal([1, 2, 3], Metronome::Converter.dump(M2, [1, 2, 3]))
  end
  ...
  def test_nested_model_dump
    ...
    models.product(inputs).each do |model, input|
      assert_equal(
        ...
        Metronome::Converter.dump(model, input)
      )
    end
  end
end
