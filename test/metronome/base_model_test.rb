...
class Metronome::Test::BaseModelTest < Minitest::Test
  ...
  def test_basic
    assert(E1.is_a?(Metronome::Converter))
    assert(A1.is_a?(Metronome::Converter))
  end

  def test_basic_coerce
    assert_pattern do
      Metronome::Converter.coerce(A1, [1.0, 2.0, 3.0]) => [1, 2, 3]
    end

    assert_pattern do
      Metronome::Converter.coerce(A2, %w[a b c]) => [:a, :b, :c]
    end
  end

  def test_basic_dump
    assert_pattern do
      Metronome::Converter.dump(A1, [1.0, 2.0, 3.0]) => [1, 2, 3]
    end

    assert_pattern do
      Metronome::Converter.dump(A2, %w[a b c]) => %w[a b c]
    end
  end
  ...
  def test_primitive_try_strict_coerce
    ...
    cases.each do |test, expect|
      type, input = test
      assert_pattern do
        Metronome::Converter.try_strict_coerce(type, input) => ^expect
      end
    end
  end
  ...
  def test_basic_enum_try_strict_coerce
    ...
    cases.each do |input, expect|
      assert_pattern do
        Metronome::Converter.try_strict_coerce(E1, input) => ^expect
      end
    end
  end
  ...
  def test_basic_array_try_strict_coerce
    ...
    cases.each do |input, expect|
      assert_pattern do
        Metronome::Converter.try_strict_coerce(A1, input) => ^expect
      end
    end
  end
  ...
  def test_nested_array_try_strict_coerce
    ...
    cases.each do |input, expect|
      assert_pattern do
        Metronome::Converter.try_strict_coerce(A2, input) => ^expect
      end
    end
  end

  class M1 < Metronome::BaseModel
    required :a, Time
  ...
  def test_basic_model_coerce
    ...
    cases.each do |input, expect|
      assert_pattern do
        Metronome::Converter.coerce(M2, input) => ^expect
      end
    end
  end
  ...
  def test_basic_model_dump
    ...
    cases.each do |input, expect|
      assert_pattern do
        Metronome::Converter.dump(M2, input) => ^expect
      end
    end
  end
  ...
  def test_basic_model_try_strict_coerce
    ...
    cases.each do |input, expect|
      assert_pattern do
        Metronome::Converter.try_strict_coerce(M1, input) => ^expect
      end
    end
  end
  ...
  def test_nested_model_dump
    ...
    models.product(inputs).each do |model, input|
      assert_pattern do
        Metronome::Converter.dump(model, input) => {a: now, renamed: "a", c: [1, 2, 3]}
      end
    end
  end

  A3 = Metronome::ArrayOf[A1]

  class M3 < M1
    optional :b, E1, api_name: :renamed_again
  end

  class U1 < Metronome::Union
    discriminator :type
    variant :a, M1
    variant :b, M3
  end

  class U2 < Metronome::Union
    variant A1
    variant A3
  end

  def test_basic_union
    assert(U1.is_a?(Metronome::Converter))
  ...
  def test_basic_discriminated_union_coerce
    ...
    cases.each do |input, expect|
      assert_pattern do
        Metronome::Converter.coerce(U1, input) => ^expect
      end
    end
  end
  ...
  def test_basic_discriminated_union_dump
    ...
    cases.each do |input, expect|
      assert_pattern do
        Metronome::Converter.dump(U1, input) => ^expect
      end
    end
  end
  ...
  def test_basic_undifferentiated_union_try_strict_coerce
    ...
    cases.each do |input, expect|
      assert_pattern do
        Metronome::Converter.try_strict_coerce(U2, input) => ^expect
      end
    end
  end

  class C1 < Metronome::BaseModel
    required :a, const: :a
  ...
  class E2 < Metronome::Enum
    A = :a
    B = :b
  end

  class U3 < Metronome::Union
    discriminator :type
    variant :a, M1
    variant :b, M3
  end
  ...
  def test_basic_eql
    assert_equal(Metronome::Unknown, Metronome::Unknown)
    refute_equal(Metronome::Unknown, Metronome::BooleanModel)
    assert_equal(Metronome::BooleanModel, Metronome::BooleanModel)
