...
class MetronomeSDK::Test::BaseModelTest < Minitest::Test
  class E1 < MetronomeSDK::Enum
    A = :a
    B = :b
  end

  A1 = MetronomeSDK::ArrayOf[-> { Integer }]
  A2 = MetronomeSDK::ArrayOf[enum: -> { E1 }]

  def test_basic
    assert(E1.is_a?(MetronomeSDK::Converter))
    assert(A1.is_a?(MetronomeSDK::Converter))
  end

  def test_basic_coerce
    assert_pattern do
      MetronomeSDK::Converter.coerce(A1, [1.0, 2.0, 3.0]) => [1, 2, 3]
    end

    assert_pattern do
      MetronomeSDK::Converter.coerce(A2, %w[a b c]) => [:a, :b, :c]
    end
  end

  def test_basic_dump
    assert_pattern do
      MetronomeSDK::Converter.dump(A1, [1.0, 2.0, 3.0]) => [1, 2, 3]
    end

    assert_pattern do
      MetronomeSDK::Converter.dump(A2, %w[a b c]) => %w[a b c]
    end
  end
  ...
  def test_primitive_try_strict_coerce
    ...
    cases.each do |test, expect|
      type, input = test
      assert_pattern do
        MetronomeSDK::Converter.try_strict_coerce(type, input) => ^expect
      end
    end
  end
  ...
  def test_basic_enum_try_strict_coerce
    ...
    cases.each do |input, expect|
      assert_pattern do
        MetronomeSDK::Converter.try_strict_coerce(E1, input) => ^expect
      end
    end
  end
  ...
  def test_basic_array_try_strict_coerce
    ...
    cases.each do |input, expect|
      assert_pattern do
        MetronomeSDK::Converter.try_strict_coerce(A1, input) => ^expect
      end
    end
  end
  ...
  def test_nested_array_try_strict_coerce
    ...
    cases.each do |input, expect|
      assert_pattern do
        MetronomeSDK::Converter.try_strict_coerce(A2, input) => ^expect
      end
    end
  end

  class M1 < MetronomeSDK::BaseModel
    required :a, Time
  ...
  def test_basic_model_coerce
    ...
    cases.each do |input, expect|
      assert_pattern do
        MetronomeSDK::Converter.coerce(M2, input) => ^expect
      end
    end
  end
  ...
  def test_basic_model_dump
    ...
    cases.each do |input, expect|
      assert_pattern do
        MetronomeSDK::Converter.dump(M2, input) => ^expect
      end
    end
  end
  ...
  def test_basic_model_try_strict_coerce
    ...
    cases.each do |input, expect|
      assert_pattern do
        MetronomeSDK::Converter.try_strict_coerce(M1, input) => ^expect
      end
    end
  end
  ...
  def test_nested_model_dump
    ...
    models.product(inputs).each do |model, input|
      assert_pattern do
        MetronomeSDK::Converter.dump(model, input) => {a: now, renamed: "a", c: [1, 2, 3]}
      end
    end
  end

  A3 = MetronomeSDK::ArrayOf[A1]

  class M3 < M1
    optional :b, E1, api_name: :renamed_again
  end

  class U1 < MetronomeSDK::Union
    discriminator :type
    variant :a, M1
    variant :b, M3
  end

  class U2 < MetronomeSDK::Union
    variant A1
    variant A3
  end

  def test_basic_union
    assert(U1.is_a?(MetronomeSDK::Converter))
  ...
  def test_basic_discriminated_union_coerce
    ...
    cases.each do |input, expect|
      assert_pattern do
        MetronomeSDK::Converter.coerce(U1, input) => ^expect
      end
    end
  end
  ...
  def test_basic_discriminated_union_dump
    ...
    cases.each do |input, expect|
      assert_pattern do
        MetronomeSDK::Converter.dump(U1, input) => ^expect
      end
    end
  end
  ...
  def test_basic_undifferentiated_union_try_strict_coerce
    ...
    cases.each do |input, expect|
      assert_pattern do
        MetronomeSDK::Converter.try_strict_coerce(U2, input) => ^expect
      end
    end
  end

  class C1 < MetronomeSDK::BaseModel
    required :a, const: :a
  ...
  class E2 < MetronomeSDK::Enum
    A = :a
    B = :b
  end

  class U3 < MetronomeSDK::Union
    discriminator :type
    variant :a, M1
    variant :b, M3
  end
  ...
  def test_basic_eql
    assert_equal(MetronomeSDK::Unknown, MetronomeSDK::Unknown)
    refute_equal(MetronomeSDK::Unknown, MetronomeSDK::BooleanModel)
    assert_equal(MetronomeSDK::BooleanModel, MetronomeSDK::BooleanModel)
