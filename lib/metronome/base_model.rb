...
module Metronome
  ...
  module Converter
    ...
    #
    # @param target [Metronome::Converter, Class]
    # @param value [Object]
    #
    # @return [Object]
    #
    ...
    def self.try_strict_coerce(target, value)
      ...
      in Class
        ...
        in [-> { _1 <= Date || _1 <= Time }, String]
          Metronome::Util.suppress(ArgumentError, Date::Error) do
  ...
  class Unknown
    ...
    # @return [Boolean]
    #
    def self.==(other) = other.is_a?(Class) && other <= Metronome::Unknown
  ...
  class BooleanModel
    ...
    # @return [Boolean]
    #
    def self.==(other) = other.is_a?(Class) && other <= Metronome::BooleanModel
  ...
  class Enum
    ...
    #
    def self.==(other)
      other.is_a?(Class) && other <= Metronome::Enum && other.values.to_set == values.to_set
    end
  ...
  class Union
    ...
    private_class_method def self.variant(key, spec = nil)
      variant_info =
        case key
        in Symbol
          [key, Metronome::Converter.type_info(spec)]
        in Proc | Metronome::Converter | Class | Hash
          [nil, Metronome::Converter.type_info(key)]
        end
    ...
    # @param value [Object]
    #
    # @return [Metronome::Converter, Class, nil]
    #
    private_class_method def self.resolve_variant(value)
      case [@discriminator, value]
      in [_, Metronome::BaseModel]
        value.class
      in [Symbol, Hash]
        key =
        ...
        resolved.nil? ? Metronome::Unknown : resolved.call
      else
        nil
      end
    end
    ...
    #
    def self.==(other)
      other.is_a?(Class) && other <= Metronome::Union && other.variants == variants
    end
    ...
    def self.coerce(value)
      if (variant = resolve_variant(value))
        return Metronome::Converter.coerce(variant, value)
      end
      ...
      known_variants.each do |_, variant_fn|
        ...
        case Metronome::Converter.try_strict_coerce(variant, value)
        in [true, coerced, _]
          return coerced
        in [false, true, score]
    ...
    def self.dump(value)
      if (variant = resolve_variant(value))
        return Metronome::Converter.dump(variant, value)
      end
  ...
  class ArrayOf
    ...
    # @return [Boolean]
    #
    def ==(other) = other.is_a?(Metronome::ArrayOf) && other.item_type == item_type
    ...
    def coerce(value)
      ...
      in Enumerable unless value.is_a?(Hash)
        value.map { |item| Metronome::Converter.coerce(type, item) }
      else
        value
      end
    end
    ...
    def dump(value)
      ...
      case value
      in Enumerable unless value.is_a?(Hash)
        value.map { |item| Metronome::Converter.dump(type, item) }.to_a
      else
        value
      end
    end
    ...
    def try_strict_coerce(value)
      ...
      in Array
        ...
        mapped =
          value.map do |item|
            case Metronome::Converter.try_strict_coerce(type, item)
            in [true, coerced, score]
              tally += score
              coerced
            in [false, true, score]
    ...
    def initialize(type_info, spec = {})
      @item_type_fn = Metronome::Converter.type_info(type_info || spec)
    end
  end
  ...
  class HashOf
    ...
    # @return [Boolean]
    #
    def ==(other) = other.is_a?(Metronome::HashOf) && other.item_type == item_type
    ...
    def coerce(value)
      ...
      in Hash
        value.to_h do |key, val|
          coerced = Metronome::Converter.coerce(type, val)
    ...
    def dump(value)
      ...
      in Hash
        value.transform_values do |val|
          Metronome::Converter.dump(type, val)
        end
      else
        value
      end
    ...
    def try_strict_coerce(value)
      ...
      in Hash
        ...
        mapped =
          value.transform_values do |val|
            case Metronome::Converter.try_strict_coerce(type, val)
            in [true, coerced, score]
              tally += score
              coerced
            in [false, true, score]
  ...
  class BaseModel
    ...
    private_class_method def self.add_field(name_sym, required:, type_info:, spec:)
      type_fn, info =
        case type_info
        in Proc | Class | Metronome::Converter
          [Metronome::Converter.type_info({**spec, union: type_info}), spec]
        in Hash
          [Metronome::Converter.type_info(type_info), type_info]
        end
      ...
      define_method(name_sym) do
        ...
        Metronome::Converter.coerce(field_type, value)
    ...
    # @private
    #
    # @param value [Metronome::BaseModel, Hash{Object=>Object}, Object]
    #
    # @return [Metronome::BaseModel, Object]
    #
    def self.coerce(value)
      case (coerced = Metronome::Util.coerce_hash(value))
      in Hash
        new(coerced)
      else
        value
    ...
    # @private
    #
    # @param value [Metronome::BaseModel, Object]
    #
    # @return [Hash{Object=>Object}, Object]
    #
    def self.dump(value)
      unless (coerced = Metronome::Util.coerce_hash(value)).is_a?(Hash)
        return value
      end
      ...
      values = coerced.filter_map do |key, val|
        ...
        in nil
          ...
          in :coerce
            ...
            target = type_fn.call
            [api_name, Metronome::Converter.dump(target, val)]
          end
        end
      end.to_h
    ...
    def self.try_strict_coerce(value)
      ...
      known_fields.each_value do |field|
        ...
        in [_, true]
          ...
          case Metronome::Converter.try_strict_coerce(target, item)
          in [true, coerced, score]
            tally += score
    ...
    #
    # @param data [Hash{Symbol=>Object}, Metronome::BaseModel]
    #
    def initialize(data = {})
      case (coerced = Metronome::Util.coerce_hash(data))
      in Hash
        @data = coerced.transform_keys(&:to_sym)
      else
        raise ArgumentError.new("Expected a #{Hash} or #{Metronome::BaseModel}, got #{data.inspect}")
      end
    end
