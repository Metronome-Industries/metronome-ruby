...
module Metronome
  ...
  class Union
    ...
    private_class_method def self.variant(key = nil, type_info = nil, enum: nil, union: nil)
      variant_info =
        ...
        in [Proc, nil, nil, nil]
          [nil, key]
        in [Class | Metronome::Converter, nil, nil, nil]
          [nil, -> { key }]
        in [Symbol, Proc, nil, nil]
          [key, type_info]
        in [Symbol, Class | Metronome::Converter, nil, nil]
          [key, -> { type_info }]
  ...
  class ArrayOf
    ...
    def initialize(spec = {})
      @items_type_fn = Metronome::Converter.type_info(spec)
    end
  end
    ...
    private_class_method def self.add_field(name_sym, required:, type_info:, spec:)
      type_fn, api_name =
        ...
        in Proc | Class | Metronome::Converter
          [Metronome::Converter.type_info({**spec, union: type_info}), spec[:api_name]]
        in Hash
          [Metronome::Converter.type_info(type_info), type_info[:api_name]]
        end
