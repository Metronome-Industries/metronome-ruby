...
module Metronome
  ...
  class BaseClient
    ...
    def validate_request(req, opts)
      ...
      case opts
      in Metronome::RequestOptions | Hash
        valid_keys = Metronome::RequestOptions.options
    ...
    def follow_redirect(request, status:, location_header:)
      uri = Metronome::Util.unparse_uri(request)
      location =
        Metronome::Util.suppress(ArgumentError) do
      ...
      # from undici
      if Metronome::Util.uri_origin(uri) != Metronome::Util.uri_origin(location)
