...
module Metronome
  ...
  class BaseClient
    ...
    PLATFORM_HEADERS = {
      "x-stainless-arch" => Metronome::Util.arch,
      "x-stainless-lang" => "ruby",
      "x-stainless-os" => Metronome::Util.os,
    ...
    class << self
      ...
      def should_retry?(status, headers:)
        coerced = Metronome::Util.coerce_boolean(headers["x-should-retry"])
        case [coerced, status]
        in [true | false, _]
          coerced
      ...
      def follow_redirect(request, status:, response_headers:)
        ...
        location =
          Metronome::Util.suppress(ArgumentError) do
            URI.join(url, response_headers["location"])
          end

        unless location
          message = "Server responded with status #{status} but no valid location header."
          raise Metronome::APIConnectionError.new(url: url, message: message)
        end
        ...
        # from undici
        if Metronome::Util.uri_origin(url) != Metronome::Util.uri_origin(location)
    ...
    def initialize(
      ...
      @base_url = Metronome::Util.parse_uri(base_url)
    ...
    private def build_request(req, opts)
      ...
      path = Metronome::Util.interpolate_path(uninterpolated_path)

      query = Metronome::Util.deep_merge(req[:query].to_h, opts[:extra_query].to_h)
      ...
      body =
        ...
        in :get | :head | :options | :trace
          ...
          Metronome::Util.deep_merge(*[req[:body], opts[:extra_body]].compact)
        end

      headers, encoded = Metronome::Util.encode_content(headers, body)
      {
        method: method,
        url: Metronome::Util.join_parsed_uri(@base_url, {**req, path: path, query: query}),
        headers: headers,
        body: encoded,
        streaming: false,
    ...
    private def retry_delay(headers, retry_count:)
      ...
      span = retry_header && Metronome::Util.suppress(ArgumentError) do
    ...
    #
    # @raise [Metronome::APIError]
    # @return [Array(Net::HTTPResponse, Enumerable)]
    #
    private def send_request(request, redirect_count:, retry_count:, send_retry_header:)
      url, headers, max_retries, timeout = request.fetch_values(:url, :headers, :max_retries, :timeout)
      input = {**request.except(:timeout), deadline: Metronome::Util.monotonic_secs + timeout}
      ...
      rescue Metronome::APIConnectionError => e
        status = e
      end
      ...
      in Metronome::APIConnectionError if retry_count >= max_retries
        raise status
      in (400..) if retry_count >= max_retries || (response && !self.class.should_retry?(
        status,
        headers: response
      ))
        decoded = Metronome::Util.decode_content(response, stream: stream, suppress_error: true)

        raise Metronome::APIStatusError.for(
          url: url,
          status: status,
          body: decoded,
          request: nil,
      ...
      in (400..) | Metronome::APIConnectionError
    ...
    #   @option req [Metronome::Converter, Class, nil] :model
    #
    #   @option req [Metronome::RequestOptions, Hash{Symbol=>Object}, nil] :options
    #
    # @param headers [Hash{String=>String}, Net::HTTPHeader]
    #
    ...
    private def parse_response(req, headers:, stream:)
      decoded = Metronome::Util.decode_content(headers, stream: stream)
      unwrapped = Metronome::Util.dig(decoded, req[:unwrap])

      case [req[:page], req.fetch(:model, Metronome::Unknown)]
      in [Class => page, _]
        page.new(client: self, req: req, headers: headers, unwrapped: unwrapped)
      in [nil, Class | Metronome::Converter => model]
    ...
    def request(req)
      ...
      Metronome::RequestOptions.validate!(opts)
    ...
    #
    def inspect
      base_url = Metronome::Util.unparse_uri(@base_url)
