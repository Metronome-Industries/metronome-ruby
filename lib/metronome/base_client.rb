...
module Metronome
  ...
  class BaseClient
    ...
    def resolve_uri_elements(req)
      from_args =
        if (url = req[:url])
          Metronome::Util.parse_uri(url)
        else
          path = Metronome::Util.normalize_path("/#{@base_path}/#{req.fetch(:path)}")
    ...
    private def send_request(
      ...
      url = Metronome::Util.unparse_uri(request)
    ...
    # @return [String]
    def inspect
      base_url = Metronome::Util.unparse_uri(scheme: @scheme, host: @host, port: @port, path: @base_path)
