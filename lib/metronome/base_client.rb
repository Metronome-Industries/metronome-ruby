...
module Metronome
  ...
  class BaseClient
    ...
    def validate_request(req, opts)
      ...
      unless opts.is_a?(Hash) || opts.is_a?(Metronome::RequestOption)
      ...
      opts.to_h.each_key do |k|
        ...
        unless (valid_keys = Metronome::RequestOptions.options).include?(k)
    ...
    def resolve_uri_elements(req)
      from_args =
        if req[:url]
          ...
          from_req[:path] = Metronome::Util.normalize_path("/#{@base_path}/#{from_req[:path]}")
          from_req
        end
